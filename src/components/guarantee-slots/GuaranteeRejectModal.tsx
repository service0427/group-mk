...
interface GuaranteeRejectModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (reason: string) => void;
}

const GuaranteeRejectModal: React.FC<GuaranteeRejectModalProps> = ({
  isOpen,
  onClose,
  onConfirm
}) => {
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-md p-0 overflow-hidden flex flex-col">
        <DialogHeader className="bg-background py-4 px-6 border-b sticky top-0 z-10">
          <DialogTitle className="text-lg font-medium text-foreground">보장형 슬롯 반려</DialogTitle>
          <DialogDescription className="sr-only">
            보장형 슬롯을 반려하고 반려 사유를 입력하는 모달입니다.
          </DialogDescription>
        </DialogHeader>

        <div className="p-6 bg-background overflow-y-auto flex-1">
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            선택한 보장형 슬롯을 반려하시겠습니까? 반려 시 사용자에게 캐시가 환불됩니다.
          </p>
...
export default GuaranteeRejectModal;