...
const SidebarContent = ({ height = 0 }: Props) => {
  ...
  return (
    <div className={`sidebar-content flex grow shrink-0 ${isMobile ? 'py-2' : 'py-3 md:py-5'} pe-2`}>
      <div
        className={`scrollable-y-hover grow shrink-0 flex ps-2 lg:ps-5 pe-1 lg:pe-3 ${isMobile ? 'mobile-sidebar-content w-full overflow-y-auto hide-scrollbar' : ''}`}
        style={{
          ...(height > 0 && { height: `${height}px` }),
          ...(isMobile && {
            height: 'calc(100vh - 40px)', // 모바일에서 높이 제한
            paddingBottom: '40px', // 하단 여백
