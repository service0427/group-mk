...
const WithdrawUserForm: React.FC<WithdrawUserFormProps> = ({users, globalSettings, onSave, selectedSetting, onCancelEdit}) => {
    ...
    return(
        <div>
            ...
            <div className="flex flex-col md:flex-row md:items-end md:space-x-4 space-y-4 md:space-y-0 p-4">
                {/* 사용자 선택 - 커스텀 드롭다운으로 변경 */}
                <div className="flex flex-col w-full md:w-auto">
                    <label className="text-sm text-gray-600 mb-1">사용자 선택</label>
                    <div className="relative">
                        <div 
                            className={`h-10 w-full md:w-[240px] px-3 border rounded-md flex items-center justify-between cursor-pointer bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 ${isEditMode ? 'bg-gray-100' : ''}`}
                            onClick={() => !isSaving && !isEditMode && setIsDropdownOpen(!isDropdownOpen)}
                        >
                            <span className={`${selectedUser ? "text-black" : "text-gray-400"} truncate max-w-[200px]`}>
                            {selectedUser 
                        ...
                        {isDropdownOpen && !isEditMode && (
                            <div className="absolute left-0 right-0 mt-1 border rounded-md shadow-lg bg-white z-10 max-h-60 overflow-y-auto">
                ...
                <div className="flex flex-col w-full md:w-auto">
                    <label className="text-sm text-gray-600 mb-1">최소 출금 금액</label>
                    <input
                        type="text"
                        name="min_request_amount"
                        value={formattedValues.min_request_amount}
                        onChange={handleChange}
                        className="h-10 px-3 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 w-full"
                        disabled={isSaving}
                        placeholder="0"
                    />
                </div>
                ...
                {/* 출금 수수료율 */}
                <div className="flex flex-col w-full md:w-auto">
                    <label className="text-sm text-gray-600 mb-1">출금 수수료율 (%)</label>
                    <input
                        type="text"
                ...
                <div className="flex space-x-2 w-full md:w-auto">
                    ...
                    <button
                        ...
                        className="h-10 px-4 bg-blue-500 text-white rounded-md hover:bg-blue-600 transition disabled:bg-blue-300 w-full md:w-auto flex-1 md:flex-none"
                        disabled={isSaving}
                    >
                    ...
                    {isEditMode && (
                        <button
                            ...
                            className="h-10 px-4 bg-gray-500 text-white rounded-md hover:bg-gray-600 transition w-full md:w-auto flex-1 md:flex-none"
                            disabled={isSaving}
                        >
                            취소
                        </button>
