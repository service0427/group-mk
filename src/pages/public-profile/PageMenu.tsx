...
// 경로 세그먼트를 분석하여 메뉴 구조에서 해당 경로에 맞는 아이템 찾기
  ...
  const findPath = (
    ...
    for (const item of items) {
      ...
      if (item.children) {
        ...
        if (exactMatch) return; // 정확한 매치를 찾았다면 더 이상 탐색하지 않음
      }
    }
  };
...
const findSiblingMenus = (menuPath: IMenuItemConfig[], currentPath: string): TMenuConfig | undefined => {
  ...
  // 네이버 플레이스 경로의 경우 특별 처리 (advertise/naver/place/...)
  ...
  // 네이버 쇼핑 경로의 경우 특별 처리 (advertise/naver/shopping/...)
  if (segments.length >= 3 &&
    segments[0] === 'advertise' &&
    segments[1] === 'naver' &&
    segments[2] === 'shopping') {
...
const determineFallbackIndex = (pathname: string): number => {
  ...
  if (pathname.includes('/advertise/naver')) {
    ...
    return 3; // 계정 관련 메뉴 (임시)
  } else if (pathname.includes('/network')) {
    return 4; // 네트워크 관련 메뉴 (임시)
  } else {
    return 2; // 기본값
  }
};
