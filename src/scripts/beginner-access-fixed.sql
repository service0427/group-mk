-- beginner-access-fixed.sql
-- 비기너 역할에 대한 접근 권한 통합 설정 스크립트
...
DECLARE
    ...
    current_table text;
BEGIN
    FOREACH current_table IN ARRAY tables_array
    LOOP
        -- 테이블 존재 여부 확인 (변수 이름 변경으로 모호성 해결)
        IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = current_table) THEN
            -- 해당 테이블의 모든 정책 삭제
            EXECUTE format('DROP POLICY IF EXISTS %1$s_for_all_users ON %1$s', current_table);
            EXECUTE format('DROP POLICY IF EXISTS %1$s_for_users ON %1$s', current_table);
            EXECUTE format('DROP POLICY IF EXISTS %1$s_insert_for_users ON %1$s', current_table);
            EXECUTE format('DROP POLICY IF EXISTS %1$s_select_policy ON %1$s', current_table);
            EXECUTE format('DROP POLICY IF EXISTS %1$s_insert_policy ON %1$s', current_table);
            
            RAISE NOTICE '% 테이블의 기존 정책이 삭제되었습니다.', current_table;
        ELSE
            RAISE NOTICE '% 테이블이 존재하지 않습니다.', current_table;
        END IF;
    END LOOP;
END $$;
