-- 환불 금액 계산을 보장 일수(guarantee_count)만 사용하도록 수정
...
BEGIN
  ...
  -- 원래 견적 요청 정보에서 보장 일수와 일일금액 가져오기
  SELECT * INTO v_request
  FROM guarantee_slot_requests
  WHERE id = v_slot.request_id;

  -- 보장 일수는 guarantee_count만 사용 (작업기간 guarantee_period는 사용하지 않음)
  v_guarantee_count := v_request.guarantee_count;
  ...
  -- 환불 금액 계산 (VAT 포함) - 보장 일수 기준으로 계산
  v_total_amount := FLOOR(v_request.final_daily_amount * v_guarantee_count * 1.1);
  ...
  v_new_request := jsonb_build_object(
    ...
    'guarantee_count', v_guarantee_count,
