-- 보장형 슬롯 환불 시 보장일수 기준으로 계산하도록 수정
-- 작성일: 2025-06-24
-- 문제: 작업기간(guarantee_period)으로 계산해서 환불 금액이 잘못 계산됨
-- 해결: 보장일수(guarantee_count) 기준으로 환불 계산
...
BEGIN
  ...
  IF v_slot.guarantee_unit = 'daily' THEN
    -- 일별 환불: 보장일수 기준으로 계산 (작업기간 아님)
    v_total_days := v_slot.guarantee_count;  -- 보장일수 사용
    v_remaining_days := GREATEST(0, v_slot.guarantee_count - v_slot.completed_count);
    
    -- 전체 환불 금액을 보장일수로 나눠서 계산
